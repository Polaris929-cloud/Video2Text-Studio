...
- **模型走国内 CDN**：首选魔搭 ModelScope（国内直连，实测 8~11MB/s），官方源与本站同源（`public/models/`）兜底；
  下载进度会显示已下载体积 / 实时速度 / 预计剩余。原因见下面「模型从哪里来」。
- **构建用相对路径**（`base: './'`），因此产物可以直接部署在 `/Video2Text-Studio/` 这样的子路径下。

### 模型从哪里来（为什么这么绕）

浏览器端加载模型踩过三个坑，这也是本项目最容易被"卡住"的地方：

1. **`huggingface.co` 在国内连不上**（TCP 被阻断）。transformers.js 内部的 fetch 默认**没有超时**，
   于是一个请求都发不出去时，界面会永远停在 0%，看起来就像死机。
2. **HF 镜像站（hf-mirror 等）在浏览器里不可用** —— 它们返回的 `Access-Control-Allow-Origin`
   只允许 `https://huggingface.co`，第三方站点跨域取模型会被 CORS 直接拦掉（对方响应头决定，客户端改不了）。
   ⚠️ 用 curl / Node 测这些地址会显示 `acao=*` 看似正常，**因为不带 `Origin` 头时它们的响应不一样** ——
   这类问题必须用真实浏览器验证，否则会误判成"镜像可用"。
3. **GitHub Pages 国内下载太慢**（实测约 100KB/s，76MB 的 Base 模型要好几分钟）；
   而 jsDelivr 这类 CDN 只能服务仓库内的文件，模型单文件 29~51MB，既超限也不适合入库。

最终方案：**首选魔搭 ModelScope**。它的路径形态正好是
`https://www.modelscope.cn/models/<模型id>/resolve/master/<文件>`，允许跨域、支持 Range 分片，
背后是国内 LFS CDN，实测 **8~11MB/s（73MB 的 Base 模型约 8 秒）**；
官方源、本站同源模型、镜像依次兜底。在此之上还加了一层保护：

- `src/lib/modelSources.ts`：模型来源候选链（魔搭 ModelScope → 本站同源 → 官方 → 镜像 → 自定义）、
  每个来源专用的路径模板、精度回退链（纯逻辑，可在 Node 单测）；
- `src/workers/asr.worker.ts`：给模型请求装上**首字节超时 / 停滞超时 / 看门狗**，每秒发一次心跳进度，
  任何阶段"没动静"都会主动中止并换下一个来源，最终给出**可读的错误**而不是无限等待；
  下载进度带上「已下载 / 总量 · 实时速度 · 预计剩余」，编译初始化阶段改用**动态条纹 + "编译中…"**
  （这段没有可上报的百分比，用假百分比会让人以为卡死）；
- **探测不等人**：来源探测一旦探到可用来源就立刻开工，其余来源在后台收尾；精度探测同理
  （主精度可用就不等 fp32）。早期版本把剩余探测全 `await` 了，结果每次启动都被连不通的
  huggingface.co 拖到超时才继续，白白多等 8 秒；
- `scripts/fetch-models.mjs`：构建时把 Tiny / Base 下载到 `public/models/` 作为同源兜底
  （CI 里跑，带缓存，失败不阻断部署；`public/models/` 不入库）；
...
1. **把「模型来源」设为「自动」**：会按 魔搭 ModelScope → 本站同源 → 官方 → 镜像 依次探测，
   并告诉你最终用了哪个、响应多快；模型建议先用 **Tiny** 或 **Base**；
2. **检查「推理设备」**：拿不到 WebGPU 适配器的环境（无头浏览器、禁用 GPU、老显卡、远程桌面）
   会在日志里提示，可在「识别设置 → 推理设备」里手动选 **CPU（WASM）**；
3. **要换别的模型**（Small / Large-v3-Turbo 等）：只要魔搭上有该模型的镜像就能直接下载；
   也可以自建一个放 ONNX 模型的静态目录，在「识别设置 → 模型来源」里选 **自定义地址** 填进去；
4. **构建自己的站点**：同源兜底模型由 `npm run fetch:models` 下载到 `public/models/`（不入库），
...
<details>
<summary><b>模型下载很慢 / 一直等在下载上？</b></summary>

模型默认从**魔搭 ModelScope**（国内直连）取，实测 8~11MB/s：Tiny（约 39MB 权重）3~5 秒、
Base（约 73MB）约 8 秒。界面上会实时显示「已下载 / 总量 · 实时速度 · 预计剩余」，
不会再出现"百分比不动、也不知道还要等多久"的情况。

如果还是很慢：

1. 把「识别设置 → 模型来源」设为 **自动**，让它按 魔搭 → 本站同源 → 官方 → 镜像 的顺序挑可用的；
2. 换 **Tiny**（权重体积只有 Base 的一半）；
3. 自己有更快的托管（对象存储 / 自建静态目录）时，选「自定义地址」填进去，
   路径需兼容 `{模型id}/resolve/main/{文件名}`；
4. 下载一次后由浏览器 `Cache Storage` 缓存，之后再用不会重复下载。

> 下载完之后还有一步「初始化推理引擎」（界面显示为动态条纹 + 编译中…），
> 那是在本机编译 WASM 推理引擎，CPU 上首次需要几十秒，与网络无关；换 Tiny 会明显更快。
</details>
